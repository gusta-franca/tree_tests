 ...
 public:
  Hasher(uint64_t maximum_value) : maximum_value_{maximum_value} {
    ...
    ASSERT(utils::is_power_of_two(maximum_value) || maximum_value == 0 || use_modulo,
           "When not using modulo-finalization, we need to map into the range [0; 2^w]. use_modulo = " +
           std::to_string(use_modulo) + ", maximum_value = " + std::to_string(maximum_value) + ", next po2 = " + 
           std::to_string(utils::value_or_np2(maximum_value)) + ".");
  }

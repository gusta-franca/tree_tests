...
#include "hashmap/hashes/buckethash.hpp" // Fingerprint and bucket bits
#include "hashmap/hashmaps/hashmap.hpp" // HashTable interface
#include "hashmap/misc/stringkey.hpp" // Not sure
#include "hashmap/simd_utils.hpp" // Low-level SIMD stuff
#include "hashmap/utils.hpp" // Mostly logging, maybe remove them and keep essential helpers
 ...
 public:
  ...
  KeyValueAoSStoringBucket(FingerprintT invalid_fingerprint) {
    DEBUG_ASSERT(number_of_fingerprints <= fps_per_vector_,
                 "Number of fingerprints " + std::to_string(number_of_fingerprints) + " not smaller than number of elements that fit in a SIMD register = " + std::to_string(fps_per_vector_) + ".");
 ...
 public:
  ...
  FindResult find(const KeyT& key) {
    ...
    if constexpr (std::is_same_v<KeyT, StringKey>) {
      DEBUG_ASSERT(bucket_idx < buckets_.size(), "For key " + std::to_string(key.string_) + ", I got bucket idx " + std::to_string(bucket_idx) + " and fingerprint " +
                                                  std::to_string(fingerprint) + ", however, we only have " + std::to_string(buckets_.size()) + " buckets!");
  ...
  std::string get_identifier() {
    ...
    if constexpr (use_prefetching) {
      prefetching = "Pref" + std::to_string(static_cast<int>(prefetching_locality));
    }
    ...
    fallback = "Fallback" + std::to_string(simd_size);
#endif

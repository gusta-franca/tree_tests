    ...
    if constexpr (std::is_same_v<KeyT, uint8_t>) {
      switch (index) {
        ...
        case 15:
          ...
          break;
        default:
          FAIL("Invalid index: " +  std::to_string(index));
      }
    }
  ...
  HEDLEY_ALWAYS_INLINE static uint32_t sve_vector_manual_first_index(
    ...
    DEBUG_ASSERT(
        tmp_vector->size() == keys_per_vector,
            "Invalid vector *size* " + tmp_vector->size() + 
            "(should be " + keys_per_vector + "). Capacity is not enough, because the SIMD instructions will not update the size information".);
